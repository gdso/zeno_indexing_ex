defmodule ZenoIndexingEx do
  ...
  To use this module, simply call `generate_key/2`
  to generated an ordered key `c` that sorts 
  lexicographically in between `a` and `b`, two ordered keys you'll provided as arguments.
  ...
  Ordered keys (or *order keys* for short) are not just any strings (see `t:order_key()`), 
    they are a concatenation of a two components: 

  1. An *integer component*, i.e. a stringified integer
  2. An optional *fractional component*, a stringified fractional value

  This order key format was first explained in Greenspan's notebook, specifically,
  his approach to _fractional indexing algorithm with logarithmic key growth_. 
  In layman's terms, this order key format was designed to slow down the growth 
  of  keys as keys are inserted in and around existing keys.

  ### TODO Integer component
  Explain and define the format of the integer component and the 
  ...
  defp has_trailing_zeroes?(nil), do: false

  defp has_trailing_zeroes?(key) when is_binary(key) do
